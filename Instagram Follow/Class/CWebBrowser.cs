...
namespace Instagram_Email_Scrape
{
    class CWebBrowser
    {
        public IWebDriver GoogleChrome(string ip, string port)
        {
            ...
            if (ip != "")
            {
                option.AddArguments("--proxy-server=" + ip + ":" + port);
            }

            option.AddExtensions("tunnel.crx");
            ...
            //new CTunnelbear().ReConnect(driver);
            return driver;
        }
